...
export class MotivosService {
  ...
  listarMotivos(): Promise<Motivos> {
    return firstValueFrom(this.http.get(`${this.motivoURL}`))
  ...
  atualizarMotivos(obj: Motivos): Promise<Motivos> {
    return firstValueFrom(this.http.put<Motivos>(`${this.motivoURL}/${obj.id}`, obj))
      .then((response) => response as Motivos);
  }

  buscarPorID(id: string){
    return firstValueFrom(this.http.get(`${this.motivoURL}/${id}`))
    .then((response) => response as Motivos);
  }

  excluir(id: string){
    return firstValueFrom(this.http.delete(`${this.motivoURL}/${id}`))
    .then(() => null);
  }
